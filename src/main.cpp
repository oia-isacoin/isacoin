...
#if defined(NDEBUG)
# error "Isacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x85ca33525e5c1534d39c9c58cff959dd483257a8615676452f4d95b47e17f97e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Isacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Isacoin Signed Message:\n";
...
{
    // Isacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 2 year
    nSubsidy >>= (nHeight / 210240); // Isacoin: 840k blocks in ~2 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Isacoin: 2 days
static const int64 nTargetSpacing = 5 * 60; // Isacoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Isacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Isacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x85ca33525e5c1534d39c9c58cff959dd483257a8615676452f4d95b47e17f97e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Birth of Isacoin on 6th of Jan 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048a521ccd2197b4a8c7c90eb3810b9a850c84f591e6fe0a569e40c7601b223da47e5b11a77ad04854510e8768feba980431c1975d5942f1cc56e039462e328bbd") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515201132;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2006567;
        ...
        assert(block.hashMerkleRoot == uint256("0x9e51022c754099d9b5e6a2115033af8dc2601d862b5515da8f0f1723ea56bd49"));
// this is where genesis generate code is
...
//this is where genesis generate code finishes
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xc4, 0xb3, 0xd2 }; // Isacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IsacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IsacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IsacoinMiner(CWallet *pwallet)
{
    printf("IsacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("isacoin-miner");
    ...
    try { loop {
        ...
        printf("Running IsacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("IsacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IsacoinMiner, pwallet));
}
